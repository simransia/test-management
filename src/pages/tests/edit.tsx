...
export default function EditTestPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (testRes.status === "success" && testRes.data) {
          ...
          reset({
            name: t.name,
            type: t.type,
            subject: t.subject, // Will map to ID later if API returns name, but let's assume it returns ID or we map it
            topics: t.topics || [],
          ...
          // Wait, if API returns name for subject, we need to find the ID.
    ...
    load();
  }, [testId, navigate, reset, setValue]);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (res.status === "success") {
        navigate(-1); // Go back to previous page (Dashboard or Publish)
      } else {
  ...
  return (
    <div className="flex h-full items-center justify-center bg-slate-900/40 p-6 overflow-y-auto">
